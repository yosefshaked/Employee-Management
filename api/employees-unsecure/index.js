...
export default async function (context, req) {
  ...
  try {
    ...
    console.log(`[API STAGE 2] Fetching plaintext dedicated key for org ${orgId}...`);
    const connectionResult = await fetchOrgPlaintextConnection(supabase, orgId);
    if (connectionResult.error) {
      ...
      const status = message === 'missing_connection_settings'
        ? 412
        : message === 'missing_plaintext_dedicated_key' || message === 'missing_plaintext_column'
          ? 428
          : 500;
      return respond(context, status, { message });
    }
    console.log('[API STAGE 2] Retrieved plaintext key and connection details.');

    console.log('[API STAGE 3] Decrypting dedicated key (lab mode bypass)...');
    const dedicatedKey = normalizeString(connectionResult.plaintextKey);
    if (!dedicatedKey) {
      return respond(context, 500, { message: 'missing_plaintext_dedicated_key' });
    }
