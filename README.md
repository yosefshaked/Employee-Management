...
- Invitation emails are delivered through Supabase admin `generateLink({ type: 'magiclink' })` responses and a SendGrid API call using the returned `action_link` so both new and existing users share the same flow.
