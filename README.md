...
- Removing an active member from **Settings → Org Members** now opens a confirmation dialog and calls the secure Azure Function `DELETE /api/memberships/{membershipId}`. The API verifies the caller is an admin or owner, deletes the `org_memberships` row, and flips the accepted invitation to `revoked` so the user can be invited again.
