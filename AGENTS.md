...
- `/api/memberships/{membershipId}` is the control-plane DELETE endpoint for removing organization members. It validates the caller as an admin/owner, deletes the `org_memberships` row, and marks matching accepted `org_invitations` as `revoked` so the member can be re-invited.
