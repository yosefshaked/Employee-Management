...
- `/api/invitations` is the control-plane API for organization invites. It uses `APP_CONTROL_DB_URL` and `APP_CONTROL_DB_SERVICE_ROLE_KEY` for the Supabase admin client, enforces admin/owner membership before writes, auto-expires pending rows once `expires_at` passes, and updates statuses to `accepted`, `declined`, `revoked`, or `expired`. When an invite target already has a Supabase user the function inserts the row and stops (the UI surfaces an in-app notification). Otherwise it sends the classic invite email via `supabase.auth.admin.inviteUserByEmail`, injecting `inviter_name` and `organization_name` metadata and an `accept-invite` redirect with the invitation token.
- Org selection pulls pending invites via `GET /api/invitations/incoming` (through `listIncomingInvitations`) and routes accept/decline actions through the secure API helpers exposed by `OrgProvider`.
