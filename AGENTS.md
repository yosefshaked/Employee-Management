...
- `/api/directory` must be used to load organization rosters and pending invites; it validates the caller via `org_memberships` with the service-role client before returning members so the frontend never queries `org_memberships` directly with the user token.
