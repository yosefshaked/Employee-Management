...
- `holiday_rules[]` – מערך של אובייקטים `{ id, name, type, start_date, end_date, recurrence }` שמגדירים טווחי חגים ומסווגים אותם כחג משולם ע"ח המערכת, חג שיורד מהמכסה, לא משולם, מעורב או ערב חג חצי יום.
...
- שורות חגים כוללות שם, טווח תאריכים ותגית מתוך:
  - `system_paid` → "חג (משולם – ע"ח המערכת)" (אין ניכוי, השכר מסומן כמשולם ע"י הארגון).
  - `employee_paid` → "חג (משולם – יורד מהמכסה)" (ניכוי מהמכסה של העובד).
  - `unpaid` → "לא משולם".
  - `mixed` → "מעורב".
  - `half_day` → "ערב חג (חצי יום)" (זמין רק כאשר חצי יום מאושר במדיניות).
