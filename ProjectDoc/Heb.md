...
- `POST /api/invitations` מקבל `{ orgId, email, expiresAt?, redirectTo?, emailData? }` ממנהלים/בעלים, בודק שאין חברות קיימת או הזמנה ממתינה ומוסיף שורה ל-`org_invitations`. לאחר אימות ההרשאה הפונקציה מחפשת את המייל ב-Supabase Auth: משתמש שקיים כבר מקבל רק את הרשומה הממתינה כדי שה-UI יציג התראה בתוך המערכת, בעוד שמייל חדש מפעיל `supabase.auth.admin.inviteUserByEmail` עם הטוקן בכתובת ההפניה ל-`/#/accept-invite` ונתוני `inviter_name`/`organization_name` להתאמה אישית של המייל.
...
- מחזור הסטטוסים: `pending` → (`accepted` | `declined` | `revoked` | `expired`). עדכון פקיעת תוקף מתבצע בצד השרת לפני המענה ללקוח.
...
### 2.5 בחירת ארגון והזמנות נכנסות

- לאחר ההתחברות `OrgProvider` קורא ל-`listIncomingInvitations()` מתוך `src/api/invitations.js`, שומר את הרשומות המטוהרות ב-`incomingInvites` ומרענן אותן אחרי כל פעולה של אישור או דחייה.
- הקונטקסט נשען כעת על `GET /api/invitations/incoming` במקום על קריאות Supabase ישירות להצגת הזמנות, כדי להשאיר את הנתונים מאחורי שכבת ה-API המאובטחת של הבקרה.
- `src/Pages/OrgSelection.jsx` מציגה כרטיס בולט של "הזמנות ממתינות" כאשר קיימות הזמנות, כולל שם הארגון, אימייל המוזמן וכפתורי אישור/דחייה המחוברים לפונקציות `acceptInvitation` ו-`declineInvitation` בקונטקסט.
- אישור הזמנה מפעיל את ה-API, מרענן את רשימת החברות ומנווט אוטומטית לארגון היעד; דחייה מעדכנת את הסטטוס בשרת מבלי לעזוב את מסך בחירת הארגון.
