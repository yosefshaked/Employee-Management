...
- פונקציית Azure ייעודית `GET /api/directory` מאמתת את החברות של הפונה באמצעות לקוח service-role ומחזירה את כל חברי הארגון וההזמנות הממתינות כך שהרענון בקונטקסט הארגון מציג מיד את החברים החדשים.
