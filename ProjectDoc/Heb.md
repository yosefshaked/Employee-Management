...
- הקובץ `src/api/invitations.js` עוטף את פונקציית Azure עם הפונקציות `createInvitation`, `listPendingInvitations`, `revokeInvitation` ואת עזרי דף הקבלה `getInvitationByToken`, `acceptInvitation`, `declineInvitation`. כל פונקציה מבצעת ולידציה למזהים/אימיילים ומחזירה הודעות שגיאה מקומיות בעת כשל.
...
- הנתיב `/#/accept-invite?token=...` מציג את `src/components/pages/AcceptInvitePage.jsx` ומאפשר למוזמנים להשלים את ההצטרפות ישירות מקישור המייל.
- בעת העלאת הדף הטוקן מפוענח מה-URL, מתבצעת קריאה ל-`getInvitationByToken`, ומוצג ספינר עד לקבלת הנתונים. טוקן לא תקף/פג תוקף מוביל לכרטיס שגיאה עם קישור חזרה להתחברות.
- ללא סשן פעיל מוצג שם הארגון ושני כפתורים: "יצירת חשבון חדש" ו"התחברות לחשבון קיים". שני המסלולים מפנים ל-`/login` תוך שמירת הטוקן ב-state של ה-Route כדי להחזיר את המשתמש לדף הקבלה לאחר ההתחברות.
- כאשר האימייל המחובר תואם לאימייל ההזמנה מוצגים כפתורי "אישור" ו"דחייה". אישור מפעיל את `acceptInvitation`, מרענן חברות דרך `OrgContext.refreshOrganizations` ומנתב ללוח הבקרה; דחייה מעדכנת סטטוס ומחזירה את המשתמש לסביבת העבודה.
- אם האימייל המחובר שונה מהאימייל שהוזמן, הדף חוסם קבלה ומציע כפתור "התנתקות והחלפת חשבון" שמבצע `signOut` ומחזיר ל-`/login` עם הטוקן כדי להיכנס בחשבון הנכון.
