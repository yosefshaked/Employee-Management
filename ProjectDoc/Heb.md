...
- `total_payment` מחושב ונשמר בכל שורה:
  ...
  - עובדים גלובליים: `rate_used / effectiveWorkingDays(employee, month)` (כל שורה מייצגת יום אחד; שדה השעות אינו משפיע).
...
ניתן להפעיל מצב **"בחר תאריכים להזנה מרובה"** בטבלה, לבחור עובדים ותאריכים, וללחוץ **"הזן"** כדי לפתוח מודאל המציג את כל התאריכים כרשימת טפסים קטנים—שורה אחת לכל תאריך ולעובד. לצד כל שדה מופיע כפתור **"העתק מהרישום הקודם"**, ובראש המודאל כפתורי **"העתק מהראשון לכל השורות"** למילוי מהיר.
...
הרכיב מאפשר להדביק טקסט או להעלות קובץ `.csv`. העובד נבחר במודל עצמו ואין לכלול עמודת עובד בקובץ.
...
עד 50 שורות מוצגות בתצוגה מקדימה עם הודעות שגיאה לשורה. ניתן להוריד CSV של השורות השגויות.

**תבנית וקידוד**

כפתור "הורד CSV להזנה" יוצר קובץ UTF-8 עם BOM כך שאקסל יציג עברית כראוי. התבנית כוללת שלוש שורות לדוגמה:
1. שעות — `שעות=8`
2. שיעור — `שירות="שם שירות לדוגמה"`, `מספר שיעורים=1`, `מספר תלמידים=1`
3. חופשה בתשלום — שאר העמודות ריקות

**כללי ולידציה**

- `date` חייב להיות תקין בפורמט ISO.
- `entry_type` חייב להיות אחד מהערכים המותרים.
- `session` דורש `service_name` תקין ותעריף ליום הנתון.
- `hours` דורש תעריף; עבור גלובלי התשלום מחושב לפי יום.
- `paid_leave` מותר רק לעובדים גלובליים ומשתמש בתעריף היומי הגלובלי.

רק שורות תקינות נשלחות לטבלת `WorkSessions`, ולאחר הייבוא מוצג סיכום של שורות שנקלטו מול שורות שנכשלו.
