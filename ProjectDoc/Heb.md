...
1. **חיבור** – הזינו את כתובת ה-URL הציבורית ואת מפתח ה-ANON של Supabase. הערכים נשמרים בשורת ההגדרות `supabase_connection` בטבלת `Settings` כך שמנהלים נוספים יוכלו לוודא את ההגדרה.
2. **החלת SQL** – הריצו את בלוק הסכימה ובלוק ה-RLS שלמטה (בסדר הזה) בעורך ה-SQL של Supabase בזמן שאתם מחוברים כבעלי הפרויקט.
3. **אימות** – לחצו על "הרץ אימות" באשף. הפונקציה `setup_assistant_diagnostics()` פועלת עם המפתח האנונימי, מדגישה רכיבים חסרים, ומעדכנת את `Settings.org_settings` עם `setup_completed: true` כשהכל ירוק.
