...
- כרטיס הדשבורד `RecentActivityTimeline` טוען את חמש הרשומות האחרונות מ-`WorkSessions`, משדך להן את העובדים הרלוונטיים ומציג סיווג אייקון/צבע/תווית באמצעות הפונקציה המרכזית `getActivityTypeDetails`.
- כרטיס פעילות אחרונה מציג כעת שלד טעינה בצורת קו-זמן ואיור ריק ידידותי בהתאם למפרט ה-UX המאושר, כדי להשאיר מנהלים מכוונים בזמן טעינת הנתונים.
- כרטיס פעילות אחרונה נבנה מחדש עם היררכיית שלושה טורים, מסילת קו-זמן וכללי מדדים תלויי הקשר כך שהשם, התג והסכום נשארים מיושרים בדיוק לפי המפרט.
