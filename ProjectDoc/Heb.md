...
*   **ניהול תצורה:**
    ...
    *   לאחר התחברות ובחירת ארגון מבוצעת בקשת `GET /api/org/<org-id>/keys` עם כותרת `Authorization`. ה-API מעביר את ה-JWT לפונקציית ה-RPC `public.get_org_public_keys` ב-Supabase, המאמתת חברות לפני החזרת פרטי החיבור של הארגון.
