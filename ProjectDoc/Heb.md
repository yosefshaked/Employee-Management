...
- הסרת חבר פעיל מציגה דיאלוג אישור חדש ושולחת בקשת `DELETE /api/memberships/{membershipId}` לפונקציה המאובטחת. הפונקציה מאמתת שהמבצע הוא מנהל/בעלים, מוחקת את השורה מ-`org_memberships` ומעדכנת את ההזמנה המאושרת לסטטוס `revoked` כדי שניתן יהיה להזמין את המשתמש מחדש בעתיד.
