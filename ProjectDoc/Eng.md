...
*   **Configuration Management:**
    *   Runtime credentials load from `public/runtime-config.json` (or an injected `window.__EMPLOYEE_MANAGEMENT_PUBLIC_CONFIG__`), a file ignored by Git so no Supabase keys are bundled into the build.
    *   Organization-specific Supabase URLs and anon keys are resolved on demand through the `/api/config` Azure Function, which requires `APP_SUPABASE_URL` and `APP_SUPABASE_SERVICE_ROLE` environment variables.
...
4.  **Prepare Runtime Configuration:**
    *   Copy `public/runtime-config.example.json` to `public/runtime-config.json`.
    *   Populate the copied file with the Supabase URL and anon key from the **app metadata** Supabase project.
    *   The runtime file is Git-ignored so the credentials remain outside the repository and production bundles.
5.  **Configure the Azure Function (for local testing):**
    *   Create `api/local.settings.json` with:
        ...
        ---------------------------------------------------------------
    *   These values are used solely by the `/api/config` function to validate memberships and must never be logged.
6.  **Run the Development App:**
    ---------------------
