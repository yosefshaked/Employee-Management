...
export default function EntryRow({
  ...
  return (
    <div className="p-4 border rounded-lg bg-slate-50 relative space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1">
          <Label>תאריך</Label>
          {readOnlyDate ? (
            <div className="bg-white p-2 rounded-md border text-right">{format(new Date(row.date), 'dd/MM/yyyy')}</div>
          ) : (
            <Popover>
        ...
        {employee.employee_type === 'hourly' && (
          <div className="space-y-1">
            ...
            <div className="flex gap-1">
              <Input type="number" step="0.1" value={row.hours} onChange={(e) => handleChange('hours', e.target.value)} required className="bg-white" />
        ...
        {employee.employee_type === 'global' && (
          <>
            ...
            <div className="space-y-1">
              ...
              <div className="flex gap-1">
                <Input type="number" step="0.1" value={row.hours} onChange={(e) => handleChange('hours', e.target.value)} className="bg-white" />
                {onCopyField && <Button variant="ghost" size="sm" onClick={() => onCopyField('hours')}>העתק מהרישום הקודם</Button>}
              </div>
            </div>
          </>
      ...
      {employee.employee_type === 'instructor' && selectedService && (
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <div className="flex gap-1">
              <Input type="number" value={row.sessions_count} onChange={(e) => handleChange('sessions_count', e.target.value)} className="bg-white" />
          ...
          {selectedService.payment_model === 'per_student' && (
            <div className="space-y-1">
              ...
              <div className="flex gap-1">
                <Input type="number" value={row.students_count} onChange={(e) => handleChange('students_count', e.target.value)} className="bg-white" />
